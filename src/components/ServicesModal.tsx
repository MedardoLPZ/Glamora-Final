...
// Export con nombre: lo importas como { Service as ModalSvc }
export type Service = {
  id: string;
  name: string;
  price: number;
  durationMin: number;
  category?: string;
  img?: string;         // dataURL o URL remota
  description?: string;
};
...
export default function ServicesModal({ isOpen, services, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (isOpen) {
      // clonar para no mutar al padre mientras editas
        ...
        <div className="p-5 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <section>
            ...
            <ul className="space-y-3">
              {rows.map(svc => (
                ...
                >
                  <img
                    src={svc.img || "https://via.placeholder.com/60x60?text=Svc"}
                    alt=""
                  ...
                  <div className="flex-1 grid sm:grid-cols-2 gap-2">
                    ...
                    <div>
                      <label className="block text-xs text-neutral-500">Image URL</label>
                      <input
                        value={svc.img || ""}
                        onChange={e => updateField(svc.id, "img", e.target.value)}
                        className="w-full rounded-lg border border-neutral-300 px-3 py-2"
                        placeholder="https://…"
                      />
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={onUploadTo(v => updateField(svc.id, "img", v))}
          ...
          <section className="pt-2 border-t border-neutral-200">
            ...
            <div className="rounded-xl border border-neutral-200 p-3 grid sm:grid-cols-2 gap-3">
              <div className="sm:col-span-2 flex items-center gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={onUploadTo(v => setAdding(a => ({ ...a, img: v })))}
                    className="text-xs"
                  />
                </div>
              </div>
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs text-neutral-500">Image URL (optional)</label>
                <input
                  value={adding.img}
                  onChange={e => setAdding(a => ({ ...a, img: e.target.value }))}
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2"
                  placeholder="https://…"
                />
              </div>
